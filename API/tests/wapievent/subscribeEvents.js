(function () {
    ...
    document.getElementById("sub_allmouse").addEventListener("click", function (e) {
        ...
        {
            epubsc.publish("sysEventSubscribe", mouseSubscribe);
            for (var i = 0; i < mouseSubscribe.length; i++)
            {
                document.getElementById("sub_" + mouseSubscribe[i]).checked = true;
                epubsc.subscribe(mouseSubscribe[i], eventMonitor.eventCounters[mouseSubscribe[i]].handler);
            }
        }
        else
        {
            epubsc.publish("sysEventUnsubscribe", mouseSubscribe);
            for (var i = 0; i < mouseSubscribe.length; i++)
            {
                document.getElementById("sub_" + mouseSubscribe[i]).checked = false;
                epubsc.unsubscribe(mouseSubscribe[i], eventMonitor.eventCounters[mouseSubscribe[i]].handler);
            }
        }
    }, false);
    ...
    document.getElementById("sub_allkey").addEventListener("click", function (e) {
        ...
        if (e.srcElement.checked)
        {
            epubsc.publish("sysEventSubscribe", keySubscribe);
            for (var i = 0; i < keySubscribe.length; i++)
            {
                document.getElementById("sub_" + keySubscribe[i]).checked = true;
                epubsc.subscribe(keySubscribe[i], eventMonitor.eventCounters[keySubscribe[i]].handler);
            }
        }
        else
        {
            epubsc.publish("sysEventUnsubscribe", keySubscribe);
            for (var i = 0; i < keySubscribe.length; i++)
            {
                document.getElementById("sub_" + keySubscribe[i]).checked = false;
                epubsc.unsubscribe(keySubscribe[i], eventMonitor.eventCounters[keySubscribe[i]].handler);
            }
        }
    }, false);
    ...
    document.getElementById("sub_alltouch").addEventListener("click", function (e) {
        ...
        if (e.srcElement.checked)
        {
            epubsc.publish("sysEventSubscribe", touchSubscribe);
            for (var i = 0; i < touchSubscribe.length; i++)
            {
                document.getElementById("sub_" + touchSubscribe[i]).checked = true;
                epubsc.subscribe(touchSubscribe[i], eventMonitor.eventCounters[touchSubscribe[i]].handler);
            }
        }
        else
        {
            epubsc.publish("sysEventUnsubscribe", touchSubscribe);
            for (var i = 0; i < touchSubscribe.length; i++)
            {
                document.getElementById("sub_" + touchSubscribe[i]).checked = false;
                epubsc.unsubscribe(touchSubscribe[i], eventMonitor.eventCounters[touchSubscribe[i]].handler);
            }
        }
    }, false);
    ...
    document.getElementById("sub_allpointer").addEventListener("click", function (e) {
        ...
        if (e.srcElement.checked)
        {
            epubsc.publish("sysEventSubscribe", pointerSubscribe);
            for (var i = 0; i < pointerSubscribe.length; i++)
            {
                document.getElementById("sub_" + pointerSubscribe[i]).checked = true;
                epubsc.subscribe(pointerSubscribe[i], eventMonitor.eventCounters[pointerSubscribe[i]].handler);
            }
        }
        else
        {
            epubsc.publish("sysEventUnsubscribe", pointerSubscribe);
            for (var i = 0; i < pointerSubscribe.length; i++)
            {
                document.getElementById("sub_" + pointerSubscribe[i]).checked = false;
                epubsc.unsubscribe(pointerSubscribe[i], eventMonitor.eventCounters[pointerSubscribe[i]].handler);
            }
        }
    }, false);

    
    // TODO: recreate this to listen for all event types
    function subscribe_(eventType) {
        epubsc.publish("sysEventSubscribe", eventType);
        epubsc.subscribe(eventType, eventMonitor.eventCounters[eventType].handler);
    }

    function unsubscribe_(eventType) {
        epubsc.publish("sysEventUnsubscribe", eventType);
        epubsc.unsubscribe(eventType, eventMonitor.eventCounters[eventType].handler);
    }
